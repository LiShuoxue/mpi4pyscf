...
def get_naoaux(gdf):
    ...
    with h5py.File(gdf._cderi, 'r') as f:
        if "j3c-kptij" in f:
            # DF generated by pyscf<2.10.0
            nkptij = f["j3c-kptij"].shape[0]
        else:
            # DF generated by pyscf>=2.10.0 # SY FIXME: might also need to change other places
            if isinstance(f["j3c"], h5py.Group):
                j3c_groups = sorted(f["j3c"].keys(), key=lambda x: int(x))
            else:
                # Single dataset (no group structure)
                j3c_groups = ["0"]
            nkptij = len(j3c_groups)
    naux_k_list = []
